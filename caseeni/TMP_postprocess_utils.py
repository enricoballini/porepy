...
def compute_and_plot_delta_displacement(idx_mu):
    ...
    exporter = pp.Exporter(
        sd, folder_name="./resultspartial/mech/" + str(idx_mu), file_name="u_and_p"
    )
...
def compute_fault_traction(idx_mu):
    """ """
    save_folder = "./resultspartial/mech/" + str(idx_mu)
...
def compute_fault_traction_nn(idx_mu):
    """ """
    save_folder = "./resultspartial/nn/" + str(idx_mu)
    ...
    u_ref =  u = np.load(
        "./resultspartial/nn/" + str(idx_mu) + "/displacement_" + str(0.0) + ".npy"
    )
    u_b_filled_ref = np.load(
        "./data/mech/"
        + str(idx_mu)
    ...
    for time in times_mech:
        ...
        u = np.load(
            "./resultspartial/nn/" + str(idx_mu) + "/displacement_" + str(time) + ".npy"
        )
        ...
        exporter.write_vtu(
             [
                 ...
                 (sd_fract, "T_on_plane_nn", T_normal_norm),

...
class ConstitutiveLawCase4(
    ...
    def intrinsic_permeability(self, subdomains: list[pp.Grid]) -> pp.ad.Operator:
        ...
        if sd.dim == 3:
            ...
            permeability = pp.ad.DenseArray(1e-8 * np.ones(sd.num_cells))
        elif sd.dim == 1:
            permeability = pp.ad.DenseArray(1e-12 * np.ones(sd.num_cells))
        else:  # 0D
    ...
    def normal_perm(self, interfaces) -> pp.ad.Operator:
        ...
        for id_intf, intf in enumerate(interfaces):
            ...
            if intf.dim == 2:
                ...
                perm[id_intf] = 2e-4 * np.ones([intf.num_cells])
            else:  # 0D
                perm[id_intf] = 2e-8 * np.ones([intf.num_cells])
    ...
    def porosity(self, subdomains: list[pp.Grid]) -> pp.ad.Operator:
        ...
        for index, sd in enumerate(subdomains):
            ...
            if sd.dim == 2:
                phi[index] = 1e-2 * np.ones([sd.num_cells])
            if sd.dim == 1:
                phi[index] = 1e-2 * np.ones([sd.num_cells])
            if sd.dim == 0:
...
if __name__ == "__main__":
    ...
    os.system("mkdir -p ./case_4-low/hu/")
    os.system("mkdir -p ./case_4-low/hu/BETA")
    folder_name = "./case_4-low/hu/visualization"

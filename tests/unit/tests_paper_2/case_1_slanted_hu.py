...
class ConstitutiveLawCase1Slanted(
    ...
    def normal_perm(self, interfaces) -> pp.ad.Operator:
        ...
        for id_intf, intf in enumerate(interfaces):
            if intf.dim == 2:
                ...
                perm[id_intf] = 0.01 * np.ones([intf.num_cells])
            else:  # 0D
                perm[id_intf] = 0.1 * np.ones([intf.num_cells])
...
class SolutionStrategyCase1Slanted(two_phase_hu.SolutionStrategyPressureMass):
...
class GeometryCase1Slanted(pp.ModelGeometry):
...
if __name__ == "__main__":
    ...
    os.system("mkdir -p ./case_1/slanted_hu/non-conforming")

    # os.system("mkdir -p ./case_1/slanted_hu_Kn" + str(Kn) + "/BETA")
    os.system("mkdir -p ./case_1/slanted_hu/non-conforming/BETA")

    # folder_name = "./case_1/slanted_hu_Kn" + str(Kn) + "/visualization"
    folder_name = "./case_1/slanted_hu/non-conforming/visualization"

...
if __name__ == "__main__":
    ...
    if case_id == 1:
        ...
        if case_type == "horizontal":
            ...
            output_file_ppu_fine = (
                "./case_1/hor_0.025/horizontal_ppu/OUTPUT_NEWTON_INFO"
            )
            output_file_hu_fine = "./case_1/hor_0.025/horizontal_hu//OUTPUT_NEWTON_INFO"
            save_folder = save_folder_root + "/horizontal_two_mesh_size"
    ...
    (
        ...
        time_steps_ppu_fine,
        time_chops_ppu_fine,
        cumulative_iterations_ppu_fine,
        global_cumulative_iterations_ppu_fine,
        last_iterations_ppu_fine,
        wasted_iterations_ppu_fine,
    ) = load_output_newton(output_file_ppu_fine)

    (
        time_hu,
        time_steps_hu_fine,
        time_chops_hu_fine,
        cumulative_iterations_hu_fine,
        global_cumulative_iterations_hu_fine,
        last_iterations_hu_fine,
        wasted_iterations_hu_fine,
    ) = load_output_newton(output_file_hu_fine)
    ...
    handles = np.ravel(np.reshape(handles_all[:4], (2, 2)), order="F")
    labels = np.ravel(np.reshape(labels_all[:4], (2, 2)), order="F")

...
my_modules_path = "../../mypythonmodulescut"
sentinel = False
for i in sys.path:
...
class ModelCaseEni:
    ...
    def run_one_simulation_no_python(
        ...
        os.system(
            ...
            + "/fluid/"
            + str(idx_mu)
            + " && runPBSEchelon -q echelon_low case2skew.DATA"
        )
    ...
    def run_one_simulation_no_python_mech(
        self,
        idx_mu,
        mu_param: np.array,
    ) -> None:
        ...
        if os.path.isfile("./data/mech/" + str(idx_mu) + "/end_file"):
            print(str(idx_mu) + "already done") # ma c'è anche sotto...
        else:
...
cd ./caseenicut/data/mech/""" + str(idx_mu) + """

...
_TYPES = {
    ...
    'C008': _DataType (siz = 8, fmt = 'c', nch = False,
                       dsk = numpy.dtype ('|S8'), mem = (
                           numpy.dtype ('<' + _STR_TYPE + '8'))),
...
def _skip_rec (fileobj, num, typ):
    ...
    # SLF added for compatibility with IX runs
    if not(fnmatch.fnmatch(typ,'C0[0-9][0-9]')):
        remaining = num * _TYPES [typ].siz
    else:
        #remaining = num * _TYPES ['CHAR'].siz * int(typ[-2:])
        remaining = num * int(typ[-2:])
...
def _read_rec (fileobj, descr):
    ...
    # SLF added for compatibility with IX runs
    if not(fnmatch.fnmatch(descr.typ,'C0[0-9][0-9]')):
        rec_typ = _TYPES [descr.typ]
    else:
        #remaining = num * _TYPES ['CHAR'].siz * int(typ[-2:])
        # EDR modified START - added the possibility to manage C0nn long names
        # rec_typ = _TYPES ['CHAR']
        nnstr   = descr.typ[-2:]
        nnsize  = int(nnstr)
        rec_typ = _DataType( siz = nnsize, fmt = 'c', nch = False,
                            dsk = numpy.dtype ('|S'+ nnstr ), mem = (
                                    numpy.dtype ('<' + _STR_TYPE + nnstr)))       
        # EDR modified STOP
...
class EclipseGrid (object):
    ...
    def __init__ (self, root):
        ...
        with EclipseFile (root, "EGRID") as egrid:
            ...
            # get index for the coarsened gridcells
            self.corsnum = egrid.get('CORSNUM')
...
def _intehead_date (intehead):
    ...
    year = intehead [66]; month = intehead [65]; day = intehead [64]
    hour = intehead [206]; minute = intehead [207]
    ...
    # geostat files from the Windows version doesn't have this field (?)
    if (len (intehead) >= 411):
        msecs = intehead [410]
    else:
        msecs = 0
...
class EclipseData (object):
    ...
    def summary_data(self, propname):
        ...
        :param propname: Name of the property to be loaded. This is on the form 'mnemonic well', e.g. 'WWIR I05'.
                        Alternatively, one propname can be either only well or only mnemonic. Then the value for all
                        mnememonics or all wells are given, e.g., propname='WWIR' returns WWIR for all wells.
        :returns:        Array of the data.
        """
        ...
        with EclipseFile(self.root, 'SMSPEC') as store:
            ...
            # SLF 10/1/2018 added for compatibility with IX run
            wellIX = store.get('NAMES')
        # SLF 10/1/2018 added for compatibility with IX run
        if well is None:
            well = wellIX
        ...
        if len(prop_elem)==1: # only well or mnemonic is given
            ...
            tmp_ind = numpy.where(mnemonic==prop_elem[0]) # find all indexes for the mnemonic
            #added for compatibility with pure IX runs
            well_upper = numpy.array([w.upper() for w in well])
            tmp_ind2 = numpy.where(well_upper==prop_elem[1].upper()) # fine all indexes for the well
            ind = tmp_ind[0][numpy.array([elem in tmp_ind2[0] for elem in tmp_ind[0]])]
        # The challenge with the summary files is that it contains vectors of values for all "ministeps" controlled by
        # the internal time stepping procedure. We are only interested in values at the report time, e.g., the final
        # "ministep". It it, however, possible to collect all values, e.g., for number of newton iterations.
        try:
            with EclipseFile(self.root, 'UNSMRY') as store:
                # Since we do not know the number of timesteps we must find how many times the PARAM keyword has been given
                total = store.cnt['PARAMS  '] # Gives number of ministeps
                vec_dat = store.get(kwd='PARAMS  ', seq = int(self.ext[1:])) # Get the final instance, need to remove 1 since pyhon
                                                             # counts from 0
        except:    
            with EclipseFile(self.root, self.ext) as store:
                # Since we do not know the number of timesteps we must find how many times the PARAM keyword has been given
                total = store.cnt['PARAMS  '] # Gives number of ministeps
                vec_dat = store.get(kwd='PARAMS  ', seq=total-1) # Get the final instance, need to remove 1 since pyhon
                                                             # counts from 0
        return vec_dat[ind]

#pylint: disable=too-few-public-methods, too-many-instance-attributes
class EclipseInit (EclipseData):
    """Read information from static data (geostat) file."""
    def __init__ (self, root, input_dict=None):
        ...
        directory = '/'.join(root.split('/')[:-1])
        files = os.listdir(directory)
        if any([filename.endswith('.INIT') for filename in files]):
            with EclipseFile (self.root, self.ext) as store:
            ...
            # KF et IPHS (1-OIL,2-WAT,3-OIL/WAT,4-GAS,5-OIL/GAS,6-GAS/WAT,7-OIL/WAT/GAS)
            # Potential phases (see eclipse fileformats/init.html)
            pot_phase = [['OIL'], ['WAT'], ['OIL', 'WAT'], ['GAS'], ['OIL', 'GAS'], ['GAS', 'WAT'], ['OIL', 'WAT', 'GAS']]
            # added SLF
            if intehead[14]>0:
                self.phases = pot_phase[intehead[14] -1]
            else:
                self.phases = pot_phase[-1]
            ...
            self.ni = int(input_dict['GridDimens'][0])   #pylint: disable=invalid-name
            self.nj = int(input_dict['GridDimens'][1])   #pylint: disable=invalid-name
            self.nk = int(input_dict['GridDimens'][2])  #pylint: disable=invalid-name
            # notice that we store the grid in C order, i.e. the dimensions
            # are swapped compared with what they are in Eclipse (Fortran)
            self.shape = (self.nk, self.nj, self.ni)
            # get the starting date of the simulation
            self.start_date = input_dict['StartDate']
...
def _report_from_OUT_file(casename):
    dir_name, file_name = os.path.split (casename)
    dir_name = '.' if dir_name == '' else dir_name
    by_date = {}
    for fname in os.listdir(dir_name):
        if fname.endswith('.OUT'):
            with open(dir_name + os.sep + fname, 'r') as f:
                lines = [line for line in f.readlines()]
                if 'INTERSECT' in lines[0]:
                    for j in range(0,len(lines)):
                        if '(EclReports)' in lines[j]:
                            templine = lines[j+2].split()
                            date = templine[5]
                            date = date.split('-')
                            time = templine[6]
                            time = time.split(':')
                            hours = time[0]
                            minutes = time[1]
                            seconds = str.format('%02d' % int(round(float(time[2]))))
                            if int(seconds)==60:
                                seconds = '00'
                                minutes = str.format('%02d' % (int(minutes)+1))
                                if int(minutes)==60:
                                    minutes = '00'
                                    hours = str.format('%02d' % (int(hours)+1))
                                    if int(hours)==24:
                                        hours = '00'
                                        date[0] = str.format('%02d' % (int(date[0])+1))
                            date_string = date[0] + " " + date [1].upper() + " " + date[2] + \
                                    " " + hours + ":" + minutes + ":" + seconds
                            report_date = datetime.datetime.strptime(date_string, '%d %b %Y %H:%M:%S')
                            report_number = int(lines[j].split()[0])
                            by_date[report_date] = report_number
        elif fname.endswith('.PRT'):
            with open(dir_name + os.sep + fname, 'r') as f:
                lines = [line for line in f.readlines()]
                if 'ECLIPSE' in lines[46]:
                    for j in range(0,len(lines)):
                        if 'SIMULATE AT' in lines[j]:
                            templine = lines[j+1].split()
                            if templine[3]=='JLY':
                                templine[3] = 'JUL'
                            date_string = templine[2] + " " + templine[3] + " " + templine[4]
                            report_date = datetime.datetime.strptime(date_string, '%d %b %Y')
                            report_number = int(templine[1])
                            by_date[report_date] = report_number
        elif fname.endswith('.REP'):
            with open(dir_name + os.sep + fname, 'r') as f:
                lines = [line for line in f.readlines()]
                for j in range(0,len(lines)):
                    templine = lines[j].split()
                    report_date = datetime.datetime.strptime(templine[1], '%d/%B/%Y')
                    report_number = int(templine[0])
                    by_date[report_date] = report_number
    return by_date
                        
class EclipseCase (object):
    """Read data for an Eclipse simulation case."""
    def __init__ (self, casename, input_dict=None):
        """
        :param casename:  Path to the case, with or without extension.
        :type casename:   str
        Added input_dict to deal with cases without INIT file
        """
        ...
        if len(self.by_date)==0:
            fname = casename.split('.')[-2]
            fname_all = '.'.join(casename.split('.')[0:-1])
            fname = fname.split('/')[-1]
            if ((fname+'.UNSMRY') in fileList):
                #getting reports steps from UNSMRY
                with EclipseFile (fname_all, 'SMSPEC') as store:
                    kw = store.get('KEYWORDS')
                kw_upper = numpy.array([k.upper() for k in kw])
                if 'DAY' in kw_upper:
                    iday = numpy.where(kw_upper=='DAY')
                    imonth = numpy.where(kw_upper=='MONTH')
                    iyear = numpy.where(kw_upper=='YEAR')
                elif 'TIME' in kw_upper:
                    itime = numpy.where(kw_upper=='TIME')
                with EclipseFile (fname_all, 'UNSMRY') as store:
                    total = store.cnt['PARAMS  '] # Gives number of steps
                    for j in range(0,total):
                        ...
                        if (len(iday)>0):
                            ...
                            date = self.start_date() + datetime.timedelta(days=date)
                        self.by_date[date] = j
            else:
                self.by_date = _report_from_OUT_file(casename)
            #sequence number has to be set
        ...
        self.init = EclipseInit (self.root,input_dict) # added input_dict to deal with cases without init file
    ...
    def atsm (self, when):  #pylint: disable=invalid-name
        """Recurrent data from summary file for a certain timestep. The result of this
        method is usually passed to function that need to calculate
        something using several properties.
    ...
    def summary_data(self, prop, when):
        ...
        # Summary data is a bit more tricky than restart data. Mostly because all specifications are given in the
        # SMSPEC file. We must therefore start by loading the mnemonic and the well associated with each data vector
    ...
    def grid (self):
        ...
        dimens = numpy.array ([self._grid.shape [2],
                               self._grid.shape [1],
                               self._grid.shape [0]], dtype = numpy.int32)
        return cp.CornerpointGrid (dimens,
                                   self._grid.coord,
                                   self._grid.zcorn,
                                   self._grid.actnum)
    ...
    def rft_data(self, well, prop):
        ...
        with EclipseFile(self.root, 'RFT') as eclf:
            ...
            for index in range(eclf.cnt['WELLETC ']):
                if eclf.get(kwd='WELLETC', seq=index)[1] == well.upper(): # well name for current index
                    assert 'R' in eclf.get(kwd='WELLETC', seq=index)[5]  # check that this well has RFT data
                    vec_dat = eclf.get(kwd=prop, seq=index)
                    break

        return vec_dat

    # TODO: write equivalent for PLT data (see ecl fileformats for details)

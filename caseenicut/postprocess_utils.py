...
def compute_and_save_fault_traction(idx_mu):
    ...
    for time in times_mech:
        ...
        np.save("./data/mech/" + str(idx_mu) + "/traction_fracture_vector_" + str(time), np.ravel(T_vect_frac, order="F"))
...
def compute_fault_traction_nn_cut(idx_mu):
    ...
    def parallel_fcn(time):
        ...
        # NN: ---------------------------------------------------------------------------
        delta_T_nn = np.load(
            "./results/nn/" + str(idx_mu) + "/traction_fracture_vector_" + str(time) + ".npy"
        )
        ...
        delta_T_vect_frac_nn = np.reshape(delta_T_nn, (dim, -1), order="F") #/ face_area
        ...
        T_tangential_y_nn = T_tangential_nn[1].T # one on-plane direction is aligned with y
        T_tangential_t_nn = np.linalg.norm(np.array([T_tangential_nn[0],T_tangential_nn[2]]).T, ord=2, axis=1) # the other tangential dir, t, is made of x and z
        T_tangential_norm_nn = np.linalg.norm(T_tangential_nn.T, ord=2, axis=1)
        cff_nn = T_tangential_norm_nn - friction_coeff * T_normal_normal_nn
        ...
        exporter.write_vtu(
             [
                 ...
                 (sd_fract, "delta_FOM_NN_minus_T_out_of_plane_signed", -delta_T_normal_normal),

...
def plt_boxplot_dict(dictionary, save_folder, y_lim_1, y_lim_2, file_name, fontsize=20):
    ...
    plt.xlabel("time $[day]$")
    plt.ylabel(r"$\epsilon_{\Delta_{\textup{CFF}}}$", fontsize=24)

    plt.grid(visible=True, which="major", axis="both", alpha=0.5)

    plt.savefig(
        save_folder + "/" + file_name + ".pdf",
        dpi=150,
        bbox_inches="tight",
        pad_inches=0.2,
    )
...
err_relative_cff_mu_time = 999999999 * np.ones(
    (test_dataset_id.shape[0], times_mech.shape[0])
)
err_area_cff_mu_time = 999999999 * np.ones(
    (test_dataset_id.shape[0], times_mech.shape[0])
)
err_relative_vs_max_cff_mu_time = 999999999 * np.ones(
    (test_dataset_id.shape[0], times_mech.shape[0])
)

for i, idx_mu in enumerate(test_dataset_id):
    print("loading " + str(idx_mu))
    err_relative_cff_mu_time[i] = np.loadtxt(
        results_folder_nn + "/" + str(idx_mu) + "/err_relative_cff"
    )
    err_area_cff_mu_time[i] = np.loadtxt(
        results_folder_nn + "/" + str(idx_mu) + "/err_area_cff"
    )
    err_relative_vs_max_cff_mu_time[i] = np.loadtxt(

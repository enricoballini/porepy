...
for idx_mu in test_dataset_id:
    ...
    vols =  np.load(
                "./data/mech/" + str(idx_mu) + "/volumes_subdomains.npy"
            )
    vols = vols[range(int(vols.shape[0]/dim))]

    for idx_dt, time in enumerate(times_mech):
        # fom: ------------------
        # T_vect_frac = np.reshape( np.load("./data/mech/" + str(idx_mu) + "/traction_fracture_vector_" + str(time) + ".npy"), (dim, -1), order="F")   
        ...
        cff_fom = np.load(results_folder_mech + "/" + str(idx_mu) + "/cff_" + str(time) + ".npy")
        ...
        cff_nn = np.load(results_folder_nn + "/" + str(idx_mu) + "/cff_" + str(time) + ".npy")

        # err: ----------------
        ref = max(np.dot(cff_fom ** 2, vols), 1e-12)
        diff = cff_fom - cff_nn # in Pa
        err_rel[idx_dt] = np.sqrt(
                        np.dot(diff**2, vols) / ref
                    ) # dimless
        # err_abs[idx_dt] = np.sqrt(
        #                 np.dot(diff**2, vols) 
        #             ) # in N
        err_abs[idx_dt] = np.sqrt(
                        np.dot(diff**2, vols) / sum(vols)
                    ) # in Pa
        ...
        np.save(results_folder_mech + "/" + str(idx_mu) + "/cff_fom_" + str(time), cff_fom)
    ...
    np.savetxt(results_folder_mech + "/" + str(idx_mu) + "/cff_fom_integrated", cff_fom_integrated_list)
...
err_rel_cff_mu_time = 999999999*np.ones((test_dataset_id.shape[0], times_mech.shape[0]))
err_abs_cff_mu_time = 999999999*np.ones((test_dataset_id.shape[0], times_mech.shape[0]))
...
cff_fom_integrated = 999999999*np.ones((test_dataset_id.shape[0], times_mech.shape[0]))
err_rel_vs_max_cff_mu_time = 999999999*np.ones((test_dataset_id.shape[0], times_mech.shape[0]))

for i, idx_mu in enumerate(test_dataset_id):
    err_rel_cff_mu_time[i] = np.loadtxt(results_folder_nn + "/" + str(idx_mu) + "/err_relative_cff")
    err_abs_cff_mu_time[i] = np.loadtxt(results_folder_nn + "/" + str(idx_mu) + "/err_area_cff") 
    ...
    cff_fom_integrated[i] = np.loadtxt(results_folder_mech + "/" + str(idx_mu) + "/cff_fom_integrated")
...
for i, idx_mu in enumerate(test_dataset_id):
    ...
    for idx_dt, time in enumerate(times_mech):
        cff_fom = np.load(results_folder_mech + "/" + str(idx_mu) + "/cff_fom_" + str(time) + ".npy") 
        cff_nn = np.load(results_folder_nn + "/" + str(idx_mu) + "/cff_nn_" + str(time) + ".npy")
    ...
    err_rel_vs_max_cff_mu_time[i] = np.loadtxt(results_folder_nn + "/" + str(idx_mu) + "/err_relative_vs_max_cff")
